...
def parse_args():
    ...
    parser.add_argument('--pretrained_path', type=str, default=None, help='Path to pretrained weights (for Phase 2 fine-tuning)')
...
def main():
    ...
    # ✨ NOUVELLE LOGIQUE : Priorité à pretrained_path (Phase 2), puis continue_training
    if hasattr(args, 'pretrained_path') and args.pretrained_path and os.path.exists(args.pretrained_path):
        print(f"🔥 PHASE 2 : Chargement des poids pré-entraînés depuis Phase 1")
        print(f"   Fichier : {args.pretrained_path}")
        model.load_state_dict(torch.load(args.pretrained_path, map_location=device))
        
        print("   Calcul du score initial du modèle chargé...")
        min_val_loss = validate(model, loaders['val'], criterion, device)
        print(f"   💪 Le modèle démarre avec une Val Loss de : {min_val_loss:.4f}")
        print(f"   ✅ Ready pour Full Fine-Tuning !")
        
    elif args.continue_training and os.path.exists(last_model_path):
